...
sparsely connected.
\eanote{Fix language node/edge/connection/gate/circuit/network.}
A ``small-world'' model, where a few ``hub'' nodes have many edges but
most nodes have few edges, has been used to describe both
neural networks~\cite{Watts:1998} and
networks of chemical reactions in the cell~\cite{Wagner:2001,Barabasi:2000}.
\emph{Modularity} is invoked as an important property for understanding the
function and evolution of biological networks:
...
%~\cite{Schrodinger:1944,Kauffman:1969,
...
%internal connections but few connections to the larger network~\cite{Mirny:2003}.
