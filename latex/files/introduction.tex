...
the function it computes is to the ideal. \textcolor{red}{An organism produces a
set of offspring, where each is the same as its parent except for mutations that}
may alter the function it computes. The performance (fitness)
measure \textcolor{red}{acting on a population of mutants} forms the basis of
...
large of class of functions -- \textcolor{red}{fixed}-degree real polynomials -- can be evolved
...
that with very simple mutations, the class of \textcolor{red}{fixed}-degree polynomials can be
evolved with respect to the squared loss~\cite{Valiant:2012-real}.

\textcolor{red}{It is currently difficult to formalize a notion of
\emph{naturalness} for mutations in these frameworks; in particular, it is
not generally understood how mutations to DNA relate to functional changes
for an organism. That said, the more direct algorithms are appealing due to the
simplicity of their mutations.  Also,}
the ``chemical computers'' of organisms may be slow, and hence,
representations that have low complexity are attractive.
\textcolor{red}{In contrast,} Feldman's general reduction from statistical query
algorithms may use arbitrarily complex representations
(polynomial-sized circuits), depending on the specific algorithm used.
In the remainder of the introduction, we first describe a particular class of
biological circuits, \emph{transcription networks}, that motivate our study.
We then frame the evolutionary question in the language of computational
...
multiplication gates and some constant inputs).\footnote{There is a
...
\emph{et al.} have an excellent survey~\cite{Donoho:2009-sparse}.
Learning the sparsest linear function is equivalent
...
the distribution seems necessary. The evolution based on optimisation algorithm
