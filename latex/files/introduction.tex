...
%%
On the other hand, direct evolutionary mechanism, not invoking the general
...
The more direct algorithms are somewhat appealing due to their simplicity of the
...
definition should be resisted~\cite{Valiant:2013-PAC}. However, another
...
representations that have low-complexity beneficial. We first describe a
...
representation classes. The nature of circuits found in biological system may
vary, but some aspects such as \emph{sparsity} are common~\vknote{Citation
required?}.  We focus on transcription networks, which are a specific class of
...
networks and other biological circuits, see~\cite{alon}; below we present a
...
resultant the protein is produced, may be regulated by a set of proteins called
...
number may very from $0-12$~\vknote{Citations: Alon's book and references
therein. See pg. 16 Chap 2.}
...
For example, presence of sugar molecules in the environment may cause specific
transcription factors to be \emph{activated}, enabling them to regulate
production of other proteins.  The produced protein could be an
\emph{end-product}, such as an enzyme that catalyzes a metabolic reaction
involving the sugar. Alternatively, it may be another transcription factor that
regulates other genes -- we view this as intermediate computation -- and may
participate in further ``computation'' to produce the end-result.
...
bind to DNA.} \todoea{Numbers regarding depth.}
...
depend only on a constant number of input variables. But we can ask the
...
arithmetic circuit with only one addition gate (alternatively a depth-two
...
et al.'s algorithm for homogeneous halfspaces under radially symmetric
...
various names in several fields\vknote{Here, cite surveys from signal
processing, OR, compressed sensing, optimisation, machine learning}. Learning
