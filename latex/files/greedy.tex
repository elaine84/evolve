...
\noindent Nevertheless, the algorithm is appealing for its simplicity and also
for the fact that it is a \emph{proper} evolutionary mechanism -- it never uses
a representation that is not a $k$-sparse linear function.
...
mutator as follows (we will define the parameters $\lambda$ and $m$ later):
%%
\begin{enumerate}
%%
...
%
\begin{enumerate}
\item {\em Scaling}: With probability $1/2$, choose $\gamma \in [-1, 1]$ uniformly at
random and let $w^\prime = \gamma w$.
%
\item {\em Adjusting}: With probability $1/2$, do the following.  Pick $i \in \NZ(w)$
...
distributions over $\reals^n$ (Definition~\ref{defn:afghanistan}).
Then, the class $\lin^k_{0, u}$ is evolvable by an evolutionary algorithm, using
the mutation algorithm described in this section, selection rule $\optsel$, and the
