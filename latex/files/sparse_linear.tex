\vknote{BEGIN: This should actually go in the previous section.}
\begin{lemma} Let $D$ be a $\Delta$-smooth bounded distribution. Let $w \in
...
\end{proof}
\vknote{END: This should actually go in the previous section.}
...
%%
\noindent{\bf Claim A}:  If $\ltwonorm{w} \geq 2 \ltwonorm{f^S}$, then with
probability at least $1/48$, $\loss_{f, D}(w^\prime) \leq \loss_{f, D}(w) -
\ltwonorm{w - f^S}^2/12$. \medskip \\
%%
\noindent{\bf Claim B}: When $\ltwonorm{w} \leq 2 \ltwonorm{w^*_S}$ and
...
%%
\noindent{\bf Claim C}: When $\ltwonorm{w - w^*_S} \leq \epsilon$, but
...
\noindent{\it Proof of Claim A}: We show that in this case, a ``scaling''
